...
class CalculateStandingsTest < ActiveSupport::TestCase
  ...
  test "numbers the standings from one, contiguously" do
    standings = CalculateStandings.call(people: ladder(3), contest_results: []).value

    assert_equal [ Standing[person_id: 1, position: 1],
                   Standing[person_id: 2, position: 2],
                   Standing[person_id: 3, position: 3] ], standings,
      "expected each person to be handed their position, not left to be counted off"
  end

  private
    # The rules are about order, so every test above reads the order out. The
    # positions themselves are asserted once, above.
    def calculate(people:, contest_results: [])
      CalculateStandings.call(people: people, contest_results: contest_results).value.map(&:person_id)
    end
