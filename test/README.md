...
**A test's path mirrors its subject's constant, not its subject's path.** `Person`
lives at `test/models/person_test.rb`. `RuboCop::Cop::Model::NoCallbacks` lives at
`test/lib/rubocop/model/no_callbacks_test.rb` — the `cop/` segment in
`lib/rubocop/cop/model/` is RuboCop's required layout, not part of the name, so it
