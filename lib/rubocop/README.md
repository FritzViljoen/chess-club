...
subclass, so a helper silently replaces the framework's method of the same name and
nothing raises. `relevant_file?`, `excluded_file?`, `message` and `cop_config` are
the ones within reach of a helper you would plausibly write — all four are public, so
check both visibilities before adding one:

```ruby
RuboCop::Cop::Base.method_defined?(:name) ||
  RuboCop::Cop::Base.private_method_defined?(:name)
```
